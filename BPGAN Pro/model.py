...
def apply_spectral_norm(module):
    for name, child_module in module.named_children():
        if isinstance(child_module, (nn.Conv2d, nn.Linear)):
            # 对卷积层和全连接层应用 SpectralNorm 类
            setattr(module, name, SpectralNorm(child_module))
        else:
            apply_spectral_norm(child_module)


class DenseLayer(nn.Module):
    def __init__(self, inp, grow_rate, bn_size, drop_rate):
        super().__init__()
        self.bn1 = nn.BatchNorm2d(inp)
        self.relu1 = nn.ReLU()
        self.conv1 = nn.Conv2d(inp, bn_size * grow_rate, kernel_size=1, stride=1, bias=False)
        self.bn2 = nn.BatchNorm2d(bn_size * grow_rate)
        self.relu2 = nn.ReLU()
        self.conv2 = nn.Conv2d(bn_size * grow_rate, grow_rate, kernel_size=3, stride=1, padding=1, bias=False)
...
class DenseBlock(nn.Module):
    def __init__(self, num_layers, inp, bn_size, grow_rate, drop_rate):
        super().__init__()
        for i in range(num_layers):
            if i == 0:
                self.layer = nn.Sequential(
                    DenseLayer(inp + i * grow_rate, grow_rate, bn_size, drop_rate)
                )
            else:
                layer = DenseLayer(inp + i * grow_rate, grow_rate, bn_size, drop_rate)
...
class GANLoss(nn.Module):
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0,l1_lambda=0):
        super(GANLoss, self).__init__()
        self.register_buffer('real_label', torch.tensor(target_real_label))
        self.register_buffer('fake_label', torch.tensor(target_fake_label))
        self.gan_mode = gan_mode
        self.l1_lambda = l1_lambda  # L1正则化参数
        if gan_mode == 'lsgan':
    ...
    def __call__(self, prediction, target_is_real):
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            # 添加L1正则化损失
            if self.l1_lambda > 0:
                l1_loss = torch.mean(torch.abs(prediction - target_tensor))
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        model += [nn.Tanh()]
        # 激活层后面添加谱归一化
        model = self.add_spectral_norm(model)
    ...
    def add_spectral_norm(self, model):
        if isinstance(model[-1], nn.Tanh):
            model[-2] = SpectralNorm(model[-2])
        else:
            model[-1] = SpectralNorm(model[-1])
        return model
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        # spectral_norm_conv1 = SpectralNorm(
        #     nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias)
        # )
        # 在这里不改变原有层名字，应用谱归一化
        # conv_block += [spectral_norm_conv1, norm_layer(dim), nn.ReLU(True)]
        ...
        # spectral_norm_conv2 = SpectralNorm(
        #     nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias)
        # )
        # 在这里不改变原有层名字，应用谱归一化
        # conv_block += [spectral_norm_conv2, norm_layer(dim)]
...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        self.model = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=False,
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        downconv = nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)    # downsample 1/2
        ...
        if outermost:
            ...
            # Insert DenseBlock
            Dense = [DenseBlock(num_layers=5, inp=inner_nc, bn_size=4, grow_rate=32, drop_rate=0.1)]
            model = down + Dense + up
        else:
            upconv = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                        kernel_size=4, stride=2,
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d):
        ...
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            ...
            use_bias = norm_layer != nn.InstanceNorm2d
        self.conv1 = nn.Conv2d(input_nc, 1, kernel_size=3, stride=1, padding=1)
        self.net = [
    ...
    def forward(self, input):
        return  self.conv1(input) + self.net(input)
...
class Model():
    ...
    def __init__(self, opt):
        ...
        self.netG_B = define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        # apply_spectral_norm(self.netG_A)
        # apply_spectral_norm(self.netG_B)
        if self.isTrain:  # define discriminators
            ...
            self.netD_B = define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            # apply_spectral_norm(self.netD_A)
            # apply_spectral_norm(self.netD_B)
