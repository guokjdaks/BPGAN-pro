...
if __name__ == '__main__':
    ...
    for epoch in range(0, opt.niter + opt.epoch_count + 1):
        ...
        for i, data in enumerate(dataset):
            ...
            if i % 1 == 0:  # 打印训练信息
                ...
                if loss_value < 2.5:  # 检查损失值是否低于2.5
                    should_stop = True  # 更新标志变量
                    break  # 跳出循环

        if should_stop:  # 检查是否需要停止训练
            print('Stopping training as loss went below 2.5')
            model.save_networks('below_2_5')  # 保存模型
            break  # 跳出外层循环

        if epoch % opt.save_epoch_freq == 0:  # 定期保存模型权重
    ...
    if not should_stop:  # 如果没有提前停止，则保存最终模型
        model.save_networks('latest')
        model.save_networks(epoch)
